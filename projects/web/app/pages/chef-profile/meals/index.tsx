                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-8 sm:gap-x-6 xl:gap-x-8">
                    {filteredMeals.map(({ mealId, type, title, description, imageUrl, createdAt }) => (
                        <MealCard
                            ...
                            imageUrl={imageUrl}
                            onInfoClick={() => setSelectedMeal({ mealId, type, title, description, imageUrl, createdAt })}
                        />
                    ))}
                </ul>

                {selectedMeal && <CookProfileMealDialog meal={selectedMeal} onClose={() => setSelectedMeal(undefined)} />}
            </div>
        </div>
    );
}
