...
export interface PEMultiSelectionProps<T> {
    labelTitle?: string;
...
export function PEMultiSelection<T>({
    labelTitle,
    options,
    selectedOptions,
    ...
    optionTitle,
    optionIdentifier,
}: PEMultiSelectionProps<T>) {
    const formattedSelectedOptions = selectedOptions.map((option) => optionTitle(option)).join(', ');
    const buttonLabel = formattedSelectedOptions === '' ? 'Keine Auswahl' : formattedSelectedOptions;

    return (
        <Listbox value={selectedOptions} onChange={selectedOptionsChanged} multiple>
            {({ open }) => (
                <>
                    {labelTitle && (
